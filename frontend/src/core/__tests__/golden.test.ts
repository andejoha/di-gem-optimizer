/**
 * Differential golden-corpus harness: the go/no-go gate for the whole
 * migration. Every case in golden/ (generated by backend/tools/gen_corpus.py
 * from the frozen Python backend) must be reproduced byte-for-byte by
 * runOptimization before any Python code is deleted.
 */
    ...
    for (const [flagKey, enableUpgrades, convert1Star] of FLAG_COMBOS) {
      it(`${caseName} [${flagKey}]`, () => {
        ...
        try {
          ...
          expect(JSON.parse(expectedRaw)).toEqual({ status: err.status, detail: err.detail });
        }
      });
    }
  }
